...
var _ = BeforeSuite(func(ctx context.Context) {
	...
	// Install the topology CRD programmatically
	installTopologyCRD(ctx, ctrlClient)
})
...
func installTopologyCRD(ctx context.Context, ctrlClient client.Client) {
	// Create the topology CRD programmatically
	topologyCRD := &apiextensionsv1.CustomResourceDefinition{
	...
	err := ctrlClient.Create(ctx, topologyCRD)
	Expect(err).NotTo(HaveOccurred(), "Failed to create topology CRD")
}
