...
func (ssn *Session) SubsetNodesFn(podGroup *podgroup_info.PodGroupInfo, isRealAllocation bool, initNodeSet node_info.NodeSet) ([]node_info.NodeSet, error) {
	...
	for _, subsetNodesFn := range ssn.SubsetNodesFns {
		...
		for _, nodeSet := range nodeSets {
			nodeSubsets, err := subsetNodesFn(podGroup, isRealAllocation, nodeSet)
			if err != nil {
				log.InfraLogger.V(6).Infof(
					"Failed to run SubsetNodes function <%v> on job <%s/%s>", subsetNodesFn, podGroup.Namespace, podGroup.Namespace)
				return nil, err
			}
