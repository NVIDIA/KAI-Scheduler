...
func getTopologyTestsMetadata() []integration_tests_utils.TestTopologyMetadata {
	return []integration_tests_utils.TestTopologyMetadata{
		...
		{
			TestTopologyBasic: test_utils.TestTopologyBasic{
				Name: "Hierarchical preferred topology - PodGroup prefers Spine, SubGroups prefer Rack - Should allocate on closest Racks",
				Topologies: []*kueuev1alpha1.Topology{
					{
						ObjectMeta: v1.ObjectMeta{
							Name: "cluster-topology",
				...
				Jobs: []*jobs_fake.TestJobBasic{
					{
						...
						RootSubGroupSet: func() *subgroup_info.SubGroupSet {
							...
							subgroupA.AddPodSet(subgroup_info.NewPodSet("pod-a-4", 1, nil))
							...
							subgroupB.AddPodSet(subgroup_info.NewPodSet("pod-b-4", 1, nil))
						...
						Tasks: []*tasks_fake.TestTaskBasic{
							...
							{
								State:        pod_status.Pending,
								SubGroupName: "pod-b-4",
							},
						},
					},
				},
				Nodes: buildEvenlyDistributedTopologyNodes(1, 2, 4, 2, 1),
				Queues: []test_utils.TestQueueBasic{
					{
						Name:               "queue0",
						ParentQueue:        "department-a",
				...
				TaskExpectedResults: map[string]test_utils.TestExpectedResultBasic{
					// All tasks should be scheduled on the same rack
					// SubgroupA should be on the same spine
					"pending_job0-0": {
						NodeName:             "node0",
						GPUsRequired:         1,
						Status:               pod_status.Binding,
					...
					// SubgroupB should be on the same spine (but different from A)
					"pending_job0-4": {
						NodeName:             "node4",
						GPUsRequired:         1,
						Status:               pod_status.Binding,
...
func buildEvenlyDistributedTopologyNodes(numZones, numSpinesPerZone, numRacksPerSpine, numNodesPerSpine, gpusPerNode int) map[string]nodes_fake.TestNodeBasic {
	...
	for z := 1; z <= numZones; z++ {
		...
		for r := 1; r <= numSpinesPerZone; r++ {
			rackLabel := fmt.Sprintf("rack%d", (z-1)*numSpinesPerZone+r)
			for s := 1; s <= numRacksPerSpine; s++ {
				spineLabel := fmt.Sprintf("spine%d", (r-1)*numRacksPerSpine+s+(z-1)*numSpinesPerZone*numRacksPerSpine)
