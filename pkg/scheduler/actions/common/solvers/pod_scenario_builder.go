...
import (
	...
	idle_gpus_filer "github.com/NVIDIA/KAI-scheduler/pkg/scheduler/actions/common/solvers/accumulated_scenario_filters/idle_gpus"
...
func NewPodAccumulatedScenarioBuilder(
	...
	topologyAwareFilter := idle_gpus_filer.NewTopologyAwareIdleGpusFilter(scenario, session.ClusterInfo.Nodes)
	if topologyAwareFilter != nil {
		scenarioFilters = append(scenarioFilters, topologyAwareFilter)
	}

	// Full cluster-level idle GPUs filter
	idleGpusScenarioFilter := idle_gpus_filer.NewIdleGpusFilter(scenario, session.ClusterInfo.Nodes)
