...
func alwaysLess(a, b interface{}) bool { return false }
...
func Test_HasTasksToAllocate(t *testing.T) {
	...
	// Now set status so ShouldAllocate returns false
...
func Test_GetTasksToAllocate(t *testing.T) {
	...
	pg.AddTaskInfo(task)
	result := GetTasksToAllocate(pg, alwaysLess, true)
	if len(result) != 1 {
...
func Test_GetTaskToAllocateWithSubGroups(t *testing.T) {
	...
	got := GetTasksToAllocate(pg, alwaysLess, true)
	if len(got) != 2 {
...
func Test_GetTasksToAllocateRequestedGPUs(t *testing.T) {
	...
	pg.AddTaskInfo(task)
	gpus, _ := GetTasksToAllocateRequestedGPUs(pg, alwaysLess, true)
	if gpus != 2 {
...
func Test_GetTasksToAllocateInitResource(t *testing.T) {
	...
	// Nil case
	res := GetTasksToAllocateInitResource(nil, alwaysLess, true)
	if !res.IsEmpty() {
	...
	pg.AddTaskInfo(task)
	r := GetTasksToAllocateInitResource(pg, alwaysLess, true)
	cpu := r.BaseResource.Get(v1.ResourceCPU)
	if cpu != 5000 {
		t.Fatalf("want cpu=5, got %v", cpu)
	}
	// Memoization/second call should return r
	newr := GetTasksToAllocateInitResource(pg, alwaysLess, true)
	if newr != r {
