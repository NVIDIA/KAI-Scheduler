...
func TestGetTasksToEvict_Table(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			job: func() *PodGroupInfo {
				pg1 := NewPodGroupInfo("pg1")
				pg1.MinAvailable = 2
				pg1.SubGroups["sg1"] = NewSubGroupInfo("sg1", 1)
				pg1.SubGroups["sg2"] = NewSubGroupInfo("sg2", 1)

				pg1.AddTaskInfo(simpleTask("pod-1", "sg1", pod_status.Running))
				pg1.AddTaskInfo(simpleTask("pod-2", "sg1", pod_status.Running))
				pg1.AddTaskInfo(simpleTask("pod-3", "sg2", pod_status.Running))
				return pg1
			}(),
			tasksToEvict: true,
			expectTasks:  []string{"pod-1"},
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			tasks, tasksToEvict := GetTasksToEvict(tt.job, alwaysLess, alwaysLess)
