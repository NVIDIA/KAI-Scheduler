...
## Motivation
Current implementation of topology scheduling is limited to a single domain attempt. This mean that when a workload required to be scheduled with topology constraint the plugin searches for a capable domain (Without certainty the job is schedulable over) and try to allocate the job over. If the attempt fails, it will not look for a different domain capable to schedule this workload. 

The uncertainty comes from the fact that the topology plugin can only have a best effort decision, as it does not aware of all scheduling constraints, such as NodeAffinity, LabelSelecot, PVC, etc...

This made us re-think about the integration of the plugin with the scheduling framework, which lead us to suggest the NodeSets concept.

## Goals
- Design a framework hook allows for multi-domain attempt in topology scheduling
...
### SubsetNodes hook
Introduce the new hook that for a given PodGroup returns a list of NodeSets
```go
type SubsetNodesFn func(PodGroup) []NodeSet
```

Such function accept PodGroup as argument and provides multiple NodeSets to attempt scheduling over.
...
Such function is accessible during cycle, as long as the session exists, and exposes the logic of all the implementing plugins.

In case of multiple plugins that implement that hook, the expected behavior is multiple layer of SubSetting.
...
During scheduling when a PodGroup is chosen to be scheduled, we call that hook before attempting to allocate any task, dividing the cluster nodes to NodeSets.

As opposed to traditional behavior, we will now attempt to schedule the PodGroup over each NodeSet, until we find one over its scheduled, and stop the iteration.
...
type SubsetNodesFn func(job *podgroup_info.PodGroupInfo, isRealAllocation bool, nodeSet node_info.NodeSet) error
```

The `isRealAllocation` allows the developer to perform different logic in case of real allocation (For example, topology plugin need to count pending pods only, or also virtually evicted pods during simulation).
