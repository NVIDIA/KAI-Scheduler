...
- **`requiredTopologyLevel`** — The level within the topology hierarchy that pods in the subgroup must share (e.g., `"topology/rack"`).
- **`preferredTopologyLevel`** — The level within the topology hierarchy that pods in the subgroup would prefer to share if possible (e.g., `"topology/rack"`).
...
- Subgroups A and B must be co-located under the same spine.
